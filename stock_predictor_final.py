...
def main():
    ...
    if hasattr(st.session_state, 'analysis_complete') and st.session_state.analysis_complete:
        ...
        if predictions is not None:
            st.subheader("📈 Enhanced Price Chart & AI Predictions")
            if beginner_mode:
                st.info("🕯️ **Chart explanation**: Blue = historical prices, Purple = AI predictions with confidence bands, Golden stars = weekly milestones, Moving averages help show trends")
            ...
            fig.update_layout(
                title={
                    'text': f"📈 {symbol} - Enhanced Price Analysis & AI Predictions",
                    'x': 0.5,
                    'xanchor': 'center',
            ...
            if beginner_mode:
                with st.expander("🔍 Chart Features Explained"):
